...
if (!_isVehicle) then {
	...
	switch(playerSide) do
	{
		case west:
		{
			...
			if(!([false,"pumpwedge",1] call life_fnc_handleInv)) exitWith {life_action_inUse = false; };
			if(_dice < 50) then
			{
				titleText[localize "STR_ISTR_Bag_Success","PLAIN"];
				life_vehicles pushBack _curTarget;
			}
			else
			{
				titleText[localize "STR_ISTR_Bag_Failed","PLAIN"];
			};
		};
		case civilian:
		{
