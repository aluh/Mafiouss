...
for "_i" from 0 to 1 step 0 do {
    ...
    if (player distance (getMarkerPos "mk_empt_prison") > 120) exitWith {
        _esc = true;
    };
...
switch (true) do {
    case (_bail): {
        ...
        player setPos (getMarkerPos "mk_empt_prison_release");

        removeUniform player;
        removeVest player;
        player addVest (life_jail_clothes select 0);
        player forceAddUniform (life_jail_clothes select 1);
        waitUntil {uniform player == life_jail_clothes select 1};
