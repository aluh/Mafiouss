...
*/
private["_inv","_lic","_licences","_near","_near_units","_mstatus","_shrt","_side","_struct"];
...

{
    ...
    if (LICENCE_VALUE(configName _x,_side)) then {
        _struct = _struct + format["%1<br/>",localize _displayName];
    };
} forEach (format["getText(_x >> 'side') isEqualTo '%1'",_side] configClasses (missionConfigFile >> "Licences"));
