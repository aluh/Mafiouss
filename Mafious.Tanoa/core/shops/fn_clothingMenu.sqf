...
*/
private["_list","_clothes","_pic","_filter","_pos","_oldPos","_oldDir","_oldBev","_flag","_shopTitle","_licence","_shopSide","_exit","_testLogic","_nearVeh","_ut1","_ut2","_ut3","_ut4","_ut5","_light"];
if (player != vehicle player) exitWith {titleText[localize "STR_NOTF_ActionInVehicle","PLAIN"];};
_exit = false;

/* Licence check & config validation */
...
_licence = M_CONFIG(getText,"Clothing",(_this select 3),"licence");
...
if (((_this select 3) == "reb" && !licence_civ_rebel)) exitWith {hint localize "STR_Shop_NotaReb"; closeDialog 0;};
if (((_this select 3) in ["cop"] && playerSide != west)) exitWith {hint localize "STR_Shop_NotaCop"; closeDialog 0;};
if (((_this select 3) in ["dive"] && !licence_civ_dive)) exitWith { hint localize "STR_Shop_NotaDive"; closeDialog 0;};
...
/* Store licence check */
if (isClass(missionConfigFile >> "Licences" >> life_clothing_store)) then {
    _flag = M_CONFIG(getText,"Licences",life_clothing_store,"side");
    _displayName = M_CONFIG(getText,"Licences",life_clothing_store,"displayName");
    if (!(LICENCE_VALUE(life_clothing_store,_flag))) exitWith {
