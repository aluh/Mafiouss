...
*/
private["_mode","_vIndex","_spawnPoints","_className","_purchasePrice","_buyMultiplier","_rentMultiplier","_colorIndex","_spawnPoint","_vehicle","_vehicleList","_shopSide","_licences","_licencesName","_exit","_initalPrice"];
...
_licences = switch (playerSide) do {
    case civilian: {(M_CONFIG(getArray,"LifeCfgVehicles",_className,"licences") select 0)};
    case west: {(M_CONFIG(getArray,"LifeCfgVehicles",_className,"licences") select 1)};
    case independent: {(M_CONFIG(getArray,"LifeCfgVehicles",_className,"licences") select 2)};
    case east: {(M_CONFIG(getArray,"LifeCfgVehicles",_className,"licences") select 3)};
};
...
_licencesName = "";
{
    if (!(_x isEqualTo "") && {!(LICENCE_VALUE(_x,_shopSide))}) then {
        _licencesName = _licencesName + localize M_CONFIG(getText,"Licences",_x,"displayName") + "<br/>";
        _exit = true;
    };
} forEach _licences;

if (_exit) exitWith {hint parseText format[(localize "STR_Shop_Veh_NoLicence")+ "<br/><br/>%1",_licencesName];closeDialog 0;};
