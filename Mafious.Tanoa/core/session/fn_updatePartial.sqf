...
switch (_mode) do {
    ...
    case 2: {
        {
            _varName = LICENCE_VARNAME(configName _x,_flag);
            _array pushBack [_varName,LICENCE_VALUE(configName _x,_flag)];
        } forEach (format["getText(_x >> 'side') isEqualTo '%1'",_flag] configClasses (missionConfigFile >> "Licences"));
    ...
    case 8: {
        ...
        _packet set[4,life_level];
    };
    case 9: {
        _packet set[2,life_introduced];
    };
    case 10: {
        _packet set[2,life_job];
    };
};
