...
/*
//DEBUG:
systemChat format["Object: %1", _curObject];
systemChat format["Distance: %1", (player distance2D _curObject)];
*/
...
if (isNull _curObject) exitWith {
    if (_isWater) then {
        ...
        if (!isNil "_fish") then {
          ...
          [] call life_fnc_gatherWaterAction;
        };
    } else {
...
if (isClass (missionConfigFile >> "CfgFarming" >> "Animals" >> typeOf _curObject)) then {
 if ((player distance2d _curObject < 5) && (playerSide isEqualTo civilian)) then {
   [_curObject] spawn life_fnc_animalAction;
 };
};
...
if (_curObject isKindOf "House_F" && {player distance _curObject < 30} || (nearestObject [[5801.02,10430.7,0],"Land_Offices_01_V1_F"]) == _curObject) exitWith {
...
if (isPlayer _curObject && _curObject isKindOf "Man") then {
    if (((_curObject getVariable ["restrained",false]) || (_curObject getVariable["playerSurrender",false])) && !dialog && playerSide isEqualTo west) then {
        [_curObject] call life_fnc_copInteractionMenu;
    };
	if (((_curObject getVariable ["restrained",false]) || (_curObject getVariable["playerSurrender",false])) && !dialog && playerSide isEqualTo civilian) then {
    ...
    if (_isVehicle) then {
        ...
        if ((typeOf _curObject) in _miscItems) then {
          ...
          if ((typeOf _curObject) isEqualTo "Land_Money_F" && {!(_curObject getVariable ["inUse",false])}) then {
            ...
            // If it's a tree or bush
            if([str _curObject,"b_"] call KRON_StrInStr || [str _curObject,"t_"] call KRON_StrInStr) then {
              if((player distance2d _curObject < 5) && (playerSide isEqualTo civilian)) then {
