...
if (isNull _vehicle || !(_vehicle isKindOf "Car" || _vehicle isKindOf "Air" || _vehicle isKindOf "Ship" || _vehicle isKindOf "Box_IND_Grenades_F" || _vehicle isKindOf "B_supplyCrate_F" || _vehicle isKindOf "House_F")) exitWith {}; //Either a null or invalid vehicle type.
...
if (_vehicle isKindOf "Box_IND_Grenades_F" || _vehicle isKindOf "B_supplyCrate_F") then {
  ...
  if(_vehicle isKindOf "Land_Shed_02_F" || _vehicle isKindOf"Land_Shed_05_F" || _vehicle isKindOf"Land_SM_01_shed_F" || _vehicle isKindOf"Land_Warehouse_03_F" || _vehicle isKindOf"Land_Warehouse_01_F" || _vehicle isKindOf"Land_Warehouse_02_F") then
  {
    ctrlSetText[3501,getText(configFile >> "CfgWarehouses" >> (typeOf _vehicle) >> "displayName")];
  } else {
