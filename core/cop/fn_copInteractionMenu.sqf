...
//Set Check Licences Button
_Btn2 ctrlSetText localize "STR_pInAct_checkLicences";
_Btn2 buttonSetAction "[player] remoteExecCall [""life_fnc_licenceCheck"",life_pInact_curTarget]; closeDialog 0;";
...
if(!(_curTarget getVariable ["restrained",false]) && !(player getVariable ["isEscorting",false])) then {_Btn4 ctrlShow false;};
...
if(!(_curTarget getVariable ["restrained",false])) then {_Btn6 ctrlShow false;};

_Btn7 ctrlSetText localize "STR_pInAct_PutInCar";
_Btn7 buttonSetAction "[life_pInact_curTarget] call life_fnc_putInCar; closeDialog 0;";
if(!(_curTarget getVariable ["restrained",false])) then {_Btn7 ctrlShow false;};
...
if(!(_curTarget getVariable ["restrained",false])) then {_Btn8 ctrlShow false;};

if (FETCH_CONST(life_coplevel) < _seizeRank) then {_Btn8 ctrlEnable false;};

{
    if ((player distance (getMarkerPos _x) <30) && (_curTarget getVariable ["restrained",false])) exitWith { _Btn6 ctrlEnable true;};
