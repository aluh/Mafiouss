...
if (!isNull _killer && {_killer != _unit} && {side _killer != west} && {alive _killer}) then {
    if (vehicle _killer isKindOf "LandVehicle") then {
        ...
        //Get rid of this if you don't want automatic vehicle licence removal.
        if (!local _killer) then {
            [2] remoteExecCall ["life_fnc_removeLicences",_killer];
        };
    } else {
        ...
        if (!local _killer) then {
            [3] remoteExecCall ["life_fnc_removeLicences",_killer];
        };
    };
};
